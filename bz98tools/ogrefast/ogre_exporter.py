...
def _collect_vertex_bone_assignments(
    operator, mesh_data, ob, mesh, renormalize_weights
):
    bone_assignments = []
    invalid_group_reported = False
    invalid_bone_reported = False

    for vert in mesh.vertices:
        valid_groups = []
        for group in vert.groups:
            if group.group > len(ob.vertex_groups) - 1:
                if not invalid_group_reported:
                    operator.report(
                        {"WARNING"},
                        "Invalid vertex group detected. Check for bones and OGREID",
                    )
                    invalid_group_reported = True
                continue

            bone_index = mesh_data.get_bone_id(ob.vertex_groups[group.group].name)
            if bone_index >= 65535:
                if not invalid_bone_reported:
                    operator.report(
                        {"WARNING"},
                        "Invalid vertex group detected. Check for bones and OGREID",
                    )
                    invalid_bone_reported = True
                continue

            valid_groups.append((bone_index, group.weight))

        valid_groups.sort(key=lambda item: item[1], reverse=True)
        valid_groups = valid_groups[:3]

        if renormalize_weights and valid_groups:
            total_weight = sum(weight for _, weight in valid_groups)
            if total_weight > 0:
                valid_groups = [
                    (bone_index, weight / total_weight)
                    for bone_index, weight in valid_groups
                    if weight != 0
                ]

        for bone_index, weight in valid_groups:
            bone_assignments.append(BoneAssignmentData(vert.index, bone_index, weight))

    return bone_assignments
...
def collect_animations(
    ...
    if animdata:
        ...
        for bone in bones:
            ...
            bone_path_map[bone.name] = (
                p_bone.path_from_id("location"),
            ...
            m = (
                fix2
                @ e_bone.parent.matrix.to_3x3().transposed()
                @ e_bone.matrix.to_3x3()
                if e_bone.parent
                else fix1 @ e_bone.matrix.to_3x3()
            )
        ...
        for act in sorted(actions, key=lambda action: action.name):
            ...
            export_info_log.append(
                f"Export action {act.name}, slot {target_channelbag.slot.name_display}"
            )
...
def collect_tracks(
    ...
    for bone_name, path in bone_path_map.items():
        fc_loc_x, fc_loc_y, fc_loc_z = [
            fcurves_find(data_path=path[0], index=i) for i in range(3)
        ]
        nd_loc_x = (
            np.frompyfunc(fc_loc_x.evaluate, 1, 1)(nd_array) if fc_loc_x else nd_zeros
        )
        nd_loc_y = (
            np.frompyfunc(fc_loc_y.evaluate, 1, 1)(nd_array) if fc_loc_y else nd_zeros
        )
        nd_loc_z = (
            np.frompyfunc(fc_loc_z.evaluate, 1, 1)(nd_array) if fc_loc_z else nd_zeros
        )

        fc_rot_w, fc_rot_x, fc_rot_y, fc_rot_z = [
            fcurves_find(data_path=path[1], index=i) for i in range(4)
        ]
        nd_rot_w = (
            np.frompyfunc(fc_rot_w.evaluate, 1, 1)(nd_array) if fc_rot_w else nd_ones
        )
        nd_rot_x = (
            np.frompyfunc(fc_rot_x.evaluate, 1, 1)(nd_array) if fc_rot_x else nd_zeros
        )
        nd_rot_y = (
            np.frompyfunc(fc_rot_y.evaluate, 1, 1)(nd_array) if fc_rot_y else nd_zeros
        )
        nd_rot_z = (
            np.frompyfunc(fc_rot_z.evaluate, 1, 1)(nd_array) if fc_rot_z else nd_zeros
        )

        fc_scl_x, fc_scl_y, fc_scl_z = [
            fcurves_find(data_path=path[2], index=i) for i in range(3)
        ]
        nd_scl_x = (
            np.frompyfunc(fc_scl_x.evaluate, 1, 1)(nd_array) if fc_scl_x else nd_ones
        )
        nd_scl_y = (
            np.frompyfunc(fc_scl_y.evaluate, 1, 1)(nd_array) if fc_scl_y else nd_ones
        )
        nd_scl_z = (
            np.frompyfunc(fc_scl_z.evaluate, 1, 1)(nd_array) if fc_scl_z else nd_ones
        )
        ...
        try:
            ...
            for bone in p_bones:
                for contraint in bone.constraints:
                    if (
                        hasattr(contraint, "target")
                        and temp_scene.collection.objects.find(contraint.target.name)
                        == -1
                    ):
...
def collect_mesh(
    ...
    for submesh_index, ob in enumerate(selected_objects):
        ...
        submesh_tangent_format = tangent_format
        ...
        temp_object = (
            ob.evaluated_get(context.evaluated_depsgraph_get())
            if applyModifiers
            else ob
        )
        ...
        if not mesh.uv_layers.active:
            submesh_tangent_format = "TANGENT_0"

        if submesh_tangent_format != "TANGENT_0":
        ...
        tangent_dimensions = (
            4
            if submesh_tangent_format == "TANGENT_4"
            or submesh_tangent_format == "ALL"
            or submesh_tangent_format == "FLIPPED"
            else 3
        )
        if submesh_tangent_format != "TANGENT_0":
        ...
        if export_color and len(mesh.color_attributes) > 0:
            ...
            for k, v in vertex_colors:
                if (
                    not k.lower().startswith("alpha")
                    and v.domain == "CORNER"
                    and v.data_type == "BYTE_COLOR"
                ):
        ...
        out_nd_indices = submesh.set_vertex(
            ...
            if num_fake_pose > 0:
                ...
                for i in range(1, num_fake_pose + 1):
                    submesh.append_shapekey(
                        f"fake_pose{i}", nd_shape_keys, out_nd_indices
                    )

        bone_assignments = _collect_vertex_bone_assignments(
            operator,
            mesh_data,
            ob,
            mesh,
...
def collect_bones(
    ...
    if armature:
        ...
        fix = Matrix.Rotation(radians(90), 4, "Z") @ Matrix.Rotation(
            radians(180), 4, "X"
        )  # Fix bone axis
        ...
        for bone in data.bones:
            ...
            rest: Matrix = (
                (bone.parent.matrix_local @ fix @ rot).inverted()
                @ bone.matrix_local
                @ fix
                @ rot
                if bone.parent
                else rot @ bone.matrix_local @ fix @ rot
            )
        ...
        if export_skeleton:
            for i, bone in enumerate(
                sorted(bones, key=lambda bone: bone.id)
            ):  # Renumbering bone ID
                bone.id = i
...
def save(
    ...
    try:
        ...
        if skeleton_data:
            if export_animation:
                collect_anim_func = (
                    collect_bake_animations if is_visual_keying else collect_animations
                )
                collect_anim_func(
                    context=context,
                    export_info_log=export_info_log,
        ...
        if skeleton_data:
            serializer.save_skeleton(
                skeleton_data, os.path.join(folder, skel_filename), skeleton_version
            )
