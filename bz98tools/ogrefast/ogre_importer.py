...
def _try_apply_custom_split_normals(
    operator: Operator, me: Mesh, raw_normals, label: str, require_exact_loops: int = 0
) -> bool:
    ...
    for key in bones:
        ...
        while len(thisbone.parent_name) != 0:
            ...
            protmat = Matrix.Rotation(
                angle.valueRadians(), 3, Vector([axis.x, axis.y, axis.z])
            ).inverted()

            new_head_position = protmat.transposed() @ Vector(
                [head_position.x, head_position.y, head_position.z]
            )
            position = Vector3(
                ppos.x + new_head_position[0],
...
def set_bone_rotation(context: Context, bones: List[BoneData]):
    ...
    for bone in bones:
        ...
        euler = Matrix.Rotation(
            angle.valueRadians(), 3, Vector([axis.x, -axis.z, axis.y])
        ).to_euler()
...
def create_mesh(
    ...
    for submesh in submeshes:
        ...
        submesh_name = (
            f"{mesh_name}{submesh_name_delimiter}{submesh_index:0{submesh_count}}"
            if use_filename
            else submesh.encorded_name.decode(select_encoding, errors="replace")
        )

        material_name = (
            submesh.encorded_material.decode(select_encoding, errors="replace")
            if create_materials
            else ""
        )
        ...
        me.polygons.foreach_set(
            "loop_start", [i * 3 for i in range(submesh.face_count)]
        )
        ...
        if submesh.geometry.has_texture_coord:
            ...
            for texcorrd_index in range(nd_texcoords.shape[0]):
                uv_layer = me.attributes.new(
                    name=f"UVLayer{texcorrd_index}", type="FLOAT2", domain="CORNER"
                )
        ...
        if submesh.geometry.has_colors:
            ...
            color_data = me.attributes.new(
                name=f"Colour{submesh_index}", type="BYTE_COLOR", domain="CORNER"
            )
            color_data.data.foreach_set("color_srgb", nd_colors)
            alpha_data = me.attributes.new(
                name=f"Alpha{submesh_index}", type="BYTE_COLOR", domain="CORNER"
            )
        ...
        if import_normals and normal_mode == "custom" and submesh.geometry.has_normals:
            ...
            if noChange:
                ...
                print("Removed", submesh.face_count - len(me.loops) / 3, "faces")
                vertices = verts.reshape(-1, 3).tolist()
                face_array = [
                    faces[i : i + 3] for i in range(0, submesh.face_count * 3, 3)
                ]
                split = []
...
def match_face(face: List[int], vertices: List[int], mesh: Mesh, index: int) -> bool:
    ...
    if len(animations) > 0:
        ...
        for bone in pose_bones:
            m = (
                ...
                if bone.parent
                else (fix1 @ bone.matrix.to_3x3()).transposed()
            )
        ...
        for animation in animations:
            ...
            for track_data in animation.get_animations(
                bone_matrix_map=mat, fps=fps, round_frame=round_frames
            ):
                ...
                for i in range(nd_locations.shape[0]):
                    curve = channelbag.fcurves.new(
                        bone.path_from_id("location"), index=i
                    )
                    curve.group = group
                ...
                for i in range(nd_rotations.shape[0]):
                    curve = channelbag.fcurves.new(
                        bone.path_from_id("rotation_quaternion"), index=i
                    )
                    curve.group = group
                    curve.keyframe_points.add(length)
                ...
                if track_data.has_scale:
                    ...
                    for i in range(nd_scales.shape[0]):
                        curve = channelbag.fcurves.new(
                            bone.path_from_id("scale"), index=i
                        )
                        curve.group = group
                        curve.keyframe_points.add(length)
        ...
        if selected_skeleton:
            ...
            if len(skeleton_filename) != 0:
                ...
                if skeleton_data:
                    ...
                    selected_skeleton, bone_map = create_skeleton(
                        context=context,
...
def load_skeleton(
    ...
    try:
        ...
        if not selected_skeleton:
            skeleton_name = os.path.splitext(skeleton_filename)[0]
            selected_skeleton, _ = create_skeleton(
                context=context,
                import_info_log=import_info_log,
                skeleton_data=skeleton_data,
                skeleton_name=skeleton_name,
