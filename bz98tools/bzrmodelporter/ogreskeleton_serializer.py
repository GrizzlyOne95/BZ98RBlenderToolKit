...
BLENDMODE_SIZE = CHUNK_HEADER_SIZE + USHORT_SIZE  # blendmode
BONE_PARENT_SIZE = (
    CHUNK_HEADER_SIZE + USHORT_SIZE + USHORT_SIZE  # bone handle  # parent handle
)
...
class SkeletonAnimationBlendMode:
    ...
    ANIMBLEND_CUMULATIVE = (
        0x0001  # Animations are applied by calculating a weighted cumulative total.
    )
...
class SkeletonSerializer(OgreBaseSerializer):
    ...
    def read_file_header(self):
        ...
        if self.version not in {
            ...
            raise UnsupportedVersionError(
                f"Unsupported skeleton file version {self.version}"
            )
    ...
    def write_bone(self, bone):
        ...
        if bone.scale is not None and (
            bone.scale.x != 1.0 or bone.scale.y != 1.0 or bone.scale.z != 1.0
        ):
    ...
    def calc_bone_size(self, bone):
        ...
        if bone.scale is not None and (
            ...
            return BONE_SIZE_WITHOUT_SCALE + VECTOR3_SIZE  # scale
    ...
    def calc_animation_size(self, animation):
        ...
        if self.version == "[Serializer_v1.80]" and animation.use_base_keyframe:
            size += self.calc_animation_baseinfo_size(
                animation
    ...
    def calc_animation_track_size(self, track):
        ...
        size = CHUNK_HEADER_SIZE + USHORT_SIZE  # bone index
        for keyframe in track.keyframe_list:
            size += self.calc_animation_track_keyframe_size(keyframe)
        return size
    ...
    def calc_animation_track_keyframe_size(self, keyframe):
        ...
        if keyframe.scale is not None and (
            ...
            return KEYFRAME_SIZE_WITHOUT_SCALE + VECTOR3_SIZE  # scale
