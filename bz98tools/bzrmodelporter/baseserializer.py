...
class BaseSerializer:
    ...
    def read_quaternion_sruf_right(self):
        ...
        return Quaternion.from_array_sruf_right(
            struct.unpack(self.endiansymbol + "4f", b)
        )

    def read_quaternion_lufs_left(self):
        b = self.read_raw(QUATERNION_SIZE)
        return Quaternion.from_array_lufs_left(
            struct.unpack(self.endiansymbol + "4f", b)
        )

    def read_transform_rufp(self):
        b = self.read_raw(TRANSFORM_SIZE)
        return Transform.from_array_rufp_xyz(
            struct.unpack(self.endiansymbol + "12f", b)
        )
    ...
    def write_uint_array(self, vals, length):
        self.stream.write(
            struct.pack(self.endiansymbol + str(min(length, len(vals))) + "I", *vals)
        )
