...
class AssetResolver:
    def __init__(self, input_dirpath, dest_dirpath, resource_dir_list, act_path):
        self.input_dirpath = (
            input_dirpath  # Path to the directory of the given file to port
        )
        self.dest_dirpath = (
            dest_dirpath  # Path to the destination directory to place the new files
        )
...
def parse_args():
    ...
    def ternary(s):
        ...
        if s == "TRUE":
            ...
            raise ValueError(
                "Ternary option must be either 'True', 'False', or 'Auto'; Got '{s}'"
            )
...
if __name__ == "__main__":
    ...
    for i, filepath in enumerate(args.filepaths):
        ...
        if ext.lower() not in {".vdf", ".sdf", ".geo", ".map", ".odf"}:
            print(
                f'Invalid filepath #{i+1}: filepath must be a .vdf, .sdf, .geo, .map, or .odf file! Got "{ext}" from "{filepath}"'
            )
            continue
        ...
        settings = SettingsController(
            ...
            scope=ScopeSettings(
                ...
                transform=(
                    Transform(*args.scopetransform)
                    if args.scopetransform is not None
                    else None
                ),
        ...
        try:
            ...
            if ext.lower() == ".odf":
                ...
                print(
                    f"classLabel: {class_label}, baseName: {base_name}, nation: {nation}"
                )
