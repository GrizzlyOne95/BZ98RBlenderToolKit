...
class Animation:
    ...
    def create_track(self, bone):
        if bone.handle in self.track_map:
            raise Exception(
                f"Animation {self.name} already has a track for bone {bone}"
            )
...
class Bone:
    ...
    def add_child(self, child):
        if child.parent is not None:
            raise Exception(
                f"Could not parent bone {child} to bone {self} - Already parented to bone {child.parent}"
            )
    ...
    def remove_child(self, child):
        if child.parent != self:
            if child.parent is None:
                raise Exception(
                    f"Cannot remove child bone {child} from {self}; Bone has no parent"
                )
            else:
                raise Exception(
                    f"Cannot remove child bone {child} from {self}; Bone is a child of bone {child.parent}"
                )
        if child not in self.children:
            raise Exception(
                f"Cannot remove child bone {child} from {self}; Child is not in children"
            )
...
class Skeleton:
    ...
    def create_bone(self, name, handle, pos=None, ori=None, scale=None):
        ...
        if handle in self.bone_map:
            raise Exception(
                f"Bone with handle {handle} ('{name}' | '{self.bone_map[handle].name}') already exists!"
            )
    ...
    def root_bones(self):
        return iter(
            bone
            for bone in self.bone_map.values()
    ...
    def verify(self):
        if not AnimBlend.isvalid(self.blendmode):
            return (
                False,
                f"Animation blend mode is invalid: {AnimBlend.label(self.blendmode, full=True)}",
            )
        ...
        for handle, bone in self.bone_map.items():
            if bone is None:
                return (
                    False,
                    f"Bone map is not contiguous; Bone with handle {handle} is None",
                )
            if handle != bone.handle:
                return (
                    False,
                    f"Bone map is not consistent; Got bone {bone} from handle {handle}",
                )
            if bone.skeleton != self:
                return (
                    False,
                    f"Bone {bone} is in a different skeleton {bone.skeleton}",
                )
            ...
            if self.name_map[bone.name] != bone:
                return (
                    False,
                    f"Name map is not consistent; Got bone {self.name_map[bone.name]} from name {bone.name}",
                )
            if bone.parent is not None:
                if bone.parent not in self.bone_map.values():
                    return (
                        False,
                        f"Parent bone {bone.parent} of {bone} is not in bone list",
                    )
                if bone not in bone.parent.children:
                    return (
                        False,
                        f"Bone {bone} is not a child of its parent bone {bone.parent}",
                    )
                parent_list = []
                b = bone
                while True:
            ...
            for child in bone.children:
                if child not in self.bone_map.values():
                    return (
                        False,
                        f"Child bone {child} of bone {bone} is not in bone list",
                    )
                ...
                if child.parent != bone:
                    return (
                        False,
                        f"Bone {bone} is not the parent of its child bone {child}",
                    )
                bone_set.add(child)
