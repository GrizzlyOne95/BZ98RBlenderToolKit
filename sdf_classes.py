...
class SDFCHeader:
    ...
    def Read (self, fileContent, position):
        ...
        self.explosioneffect = str(array[10],'ascii').strip('\0')
        self.explosionsound = str(array[11],'ascii').strip('\0')
        return position + self.binlength
    def Write(self, fileHandle, position):
        buffer = bytearray(self.binlength)
        ...
        self.defensive,
        bytes(self.explosioneffect, 'ascii'), bytes(self.explosionsound, 'ascii'))
        fileHandle.write(buffer)
        return self.binlength + position
...
class GEOData:
    ...
    def Write(self, fileHandle, position):
        ...
        *self.boxhalfheight,
        ...
        self.ddr,self.x,self.y,self.z,self.time)
        fileHandle.write(buffer)
        return position + self.binlength
