"""
Batch-export per-bone chunk payload meshes from a Battlezone Redux `.mesh`.
...
The script imports the source mesh through the toolkit's existing Ogre path,
isolates geometry by exact vertex-group membership, shifts each extracted piece
so its local origin matches the corresponding bone head, strips armature
...
def _bootstrap_toolkit(repo_root: Path):
    ...
    # Blender may already have an installed addon copy of bz98tools loaded from
    # AppData. For batch testing we want the repo checkout to win consistently.
